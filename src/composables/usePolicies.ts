...
interface UsePoliciesResult {
  ...
  allRules: Readonly<{ value: RuleRow[] }>;
...
export function usePolicies(options: UsePoliciesOptions = {}): UsePoliciesResult {
    ...
    filteredRules.value = rawRows.value.filter((row) => {
      ...
      return (
        ...
        row.comments.some((comment) =>
          ...
          (comment.user && comment.user.toLowerCase().includes(search)) ||
          (comment.date && comment.date.toLowerCase().includes(search))
        ) ||
  ...
  const allRules = computed(() => rawRows.value);
...
export function flattenPolicies(policies: RawPolicy[]): RuleRow[] {
